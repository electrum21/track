    ...
    <>
      <nav className="bg-white dark:bg-gray-950 border-b border-gray-200 dark:border-gray-800 px-4 sm:px-6 py-3">
        <div className="flex items-center justify-between">
          <span className="hover-wiggle inline-block font-medium text-gray-900 dark:text-gray-100 cursor-default">NTUTrack</span>
          ...
          <div className="hidden md:flex absolute left-1/2 -translate-x-1/2 gap-1">
            {links.map(link => (
              <Link
                key={link.path}
                to={link.path}
                className={`relative text-sm px-3 py-1.5 rounded-lg transition-all duration-150 flex items-center gap-1.5 hover:scale-105 active:scale-95 ${
              ...
              >
                ...
                {link.badge && reviewCount > 0 && (
                  <span className="animate-badge-pulse inline-flex items-center justify-center h-4 min-w-4 px-1 text-[10px] font-semibold rounded-full bg-amber-400 dark:bg-amber-500 text-white leading-none">
                    {reviewCount}
                  </span>
                )}
              </Link>
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              title="Settings"
              className="w-8 h-8 hidden md:flex items-center justify-center rounded-lg border border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-800 text-gray-500 dark:text-gray-400 transition-all duration-150 cursor-pointer overflow-hidden hover:scale-105 active:scale-95"
            >
              {user?.photoURL ? (
                <img src={user.photoURL} className="w-full h-full object-cover" alt="" />
              ) : (
                <svg className="hover-spin" width="14" height="14" viewBox="0 0 16 16" fill="none">
            ...
            >
              {reviewCount > 0 && !mobileMenuOpen && (
                <span className="animate-badge-pulse absolute -top-1 -right-1 w-3.5 h-3.5 bg-amber-400 rounded-full border-2 border-white dark:border-gray-950" />
              )}
              {mobileMenuOpen ? (
          ...
          <div className="md:hidden mt-3 border-t border-gray-100 dark:border-gray-800 pt-3 pb-1 flex flex-col gap-0.5">
            ...
            <div className="flex items-center justify-between px-3 py-2.5 mt-1 border-t border-gray-100 dark:border-gray-800">
              <button
                ...
                className="flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-100 transition-colors cursor-pointer"
              >
                {user?.photoURL ? (
